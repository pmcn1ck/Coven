...
{
    public class DiseasedMoveAction : AIAction
    {
        ...
        public override IEnumerator Execute(Player player, Unit unit, CellGrid cellGrid)
        {
            ...
            unit.GetComponent<DiseasedMoveAbility>().Destination = TopDestination;
            unit.GetComponent<DiseasedMoveAbility>().AIExecute(cellGrid);
            while (unit.IsMoving)
            {
                yield return 0;
            }
