    ...
    class GridHelper : EditorWindow
    {
        ...
        private void MapGenerationGUI()
        {
            ...
            foreach (var field in generators[generatorIndex].GetFields().Where(f => f.IsPublic))
            {
                ...
                else if (field.FieldType == typeof(GameObject))
                {
                    ...
                    shouldDisplayCollider2DWarning = mapTypes[mapTypeIndex].Equals(MAP_TYPE_3D) && g != null && g.GetComponent<Collider2D>() != null;
                    shouldDisplayNoColliderOnCellWarning = g != null && (g.GetComponent<Collider>() == null && g.GetComponent<Collider2D>() == null);
                }
            }
            ...
            if (shouldDisplayNoColliderOnCellWarning)
            {
                GUIStyle style = new GUIStyle(EditorStyles.wordWrappedLabel);
                style.fontStyle = FontStyle.Bold;
                style.normal.textColor = Color.red;
        ...
        void GenerateBaseStructure()
        {
            ...
            for (int i = 0; i < nComputerPlayer; i++)
            {
                ...
                aiPlayer.AddComponent<SubsequentUnitSelection>();
