...
namespace TbsFramework.EditorUtils.GridGenerators
{
    ...
    public class RectangularSquareGridGenerator : ICellGridGenerator
    {
        ...
        public override GridInfo GenerateGrid()
        {
            ...
            gridInfo.Dimensions = Is2D ? new Vector3(squareSize.x * (Width - 1), squareSize.y * (Height - 1)) : new Vector3(squareSize.x * (Width - 1), squareSize.y, squareSize.z * (Height - 1));
            gridInfo.Center = Vector2.zero;
            ...
            for (int i = 0; i < Width; i++)
            {
                for (int j = 0; j < Height; j++)
                {
                    ...
                    square.transform.position = position - offset;
